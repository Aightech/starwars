...
const Color ElementModel::s_color = ElementModel_COLOR;
const Texture* ElementModel::s_texture = new Texture();
const Texture* ElementModel::s_textureTarget = new Texture();



ElementModel::ElementModel(int no, int px, int py,  Player * player):Element(no,px,py,player)
{
	m_width = s_width;
	m_height = s_height;
	m_HP = s_HP;
...
void ElementModel::setting()
{
	ElementModel:setTexture();
	Element::elements()[ElementModel_TYPE] = new ElementModel();
};

void ElementModel::setTexture()
{
	Image im;
	if (!im.loadFromFile("media/elements/ElementModel2.png"))
	{
...
Request ElementModel::reqMoveToTarget()
{
...
int  ElementModel::move(int dx,int dy)
{
